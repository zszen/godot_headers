...
func ingame_ready():
	...
	for i in range(0, array.size() - 1):
		...
		rectangle.set_extents(Vector2(Width / 2, (next - current).length() * 0.9))
		shape.shape = rectangle
		body.add_child(shape)
		body.rotation = ((next - current).angle())
	...
	for i in range(0, bodies.size() - 1):
		...
		if(last != null):
			...
			if(angle < 120 && angle > -120):
			    ...
			    var current_vector = Vector2(0, 1).rotated(current.rotation)
...
func get_end_pos(rigid_bodie):
	...
	position = position.rotated(rigid_bodie.rotation)
